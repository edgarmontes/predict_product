...
def variable_normalization(dataframe):
...
def select_kbest(X_df,y_df):
    ...
    X_df : the predictors, this dataset comes from the 
    y_df : the target
    ...
    for n_features in range(1,len(X_df.columns)):
        feature_selector = SelectKBest(score_func=f_regression, k= n_features)
        feature_selector.fit(X_df, y_df)
        mask = feature_selector.get_support()
        X_df.columns[mask]
        top_columns = list(X_df.columns[mask])
        cols = str(top_columns)
        #df_feature_selection.loc[n_features,"columns_number"] = n_features
        #df_feature_selection.loc[n_features,"columns_list"] = cols       
        print(f"The top {n_features} selected feautures based on the SelectKBest class are: {cols}")
